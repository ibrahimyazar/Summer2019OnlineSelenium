package test.day21_GitBranchOverView;
...
#####################################
GIT BRANCHES
What is branch? It's a one version of your project. With git, we can have multiple version of project that exist in parallel.
Every branch can have independent commits history.
Ideally, everyone works on their own branch, and then once work is done, you can create pull request to apply your changes to the main branch.
This flow, allows to review code before accepting ant=y changes.
...
I would commit my changes, whenever I completed automation of some user story.
Then I would add commit message like: "completed automation of VYT-4321 Driver should be able to add fuel logs".
It's not only about scripts, you can also add utilities, fix something...
...
Code review will improve code quality. Also, we will make sure that master branch has 100% working code.
 ...
 * least one assertion failed.
 ...
 softAssert.assertTrue();
    In testng there  are 2 types of assertions: soft and hard. Until now, we were using only hard assertion.
    The main difference is when condition fails, soft assertion will not stop execution until reaching assertAll() method call.
    Hard assertion, will stop the test in case of failure.
    @AfterMethod
